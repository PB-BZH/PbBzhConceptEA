//+------------------------------------------------------------------+
//|          VirtualGoldMomentumPositionManager.mqh                 |
//|  Momentum GOLD LONG, coûts et rapport de robustesse automatique |
//+------------------------------------------------------------------+
#ifndef PB_BZH_VIRTUAL_GOLD_MOMENTUM_POSITION_MANAGER_MQH
#define PB_BZH_VIRTUAL_GOLD_MOMENTUM_POSITION_MANAGER_MQH

enum ENUM_PB_V501_POSITION_DIRECTION {
  PB_V501_POSITION_FLAT = 0,
  PB_V501_POSITION_LONG = 1,
  PB_V501_POSITION_SHORT = -1
};

enum ENUM_PB_V501_EXIT_MODE {
  PB_V501_EXIT_MODE_MOMENTUM_REVERSAL = 0
};

enum ENUM_PB_V501_EXIT_REASON {
  PB_V501_EXIT_NONE = 0,
  PB_V501_EXIT_STOP_INITIAL,
  PB_V501_EXIT_MOMENTUM_REVERSAL,
  PB_V501_EXIT_TEST_END
};

string V501PositionDirectionToString(
  const ENUM_PB_V501_POSITION_DIRECTION direction) {

  switch (direction) {
    case PB_V501_POSITION_LONG:
      return "LONG";

    case PB_V501_POSITION_SHORT:
      return "SHORT";

    case PB_V501_POSITION_FLAT:
    default:
      return "FLAT";
  }
}

string V501ExitModeToString(
  const ENUM_PB_V501_EXIT_MODE mode) {
  return "MOMENTUM_REVERSAL";
}

string V501ExitReasonToString(
  const ENUM_PB_V501_EXIT_REASON reason) {

  switch (reason) {
    case PB_V501_EXIT_STOP_INITIAL:
      return "STOP_INITIAL";

    case PB_V501_EXIT_MOMENTUM_REVERSAL:
      return "MOMENTUM_REVERSAL";

    case PB_V501_EXIT_TEST_END:
      return "TEST_END";

    case PB_V501_EXIT_NONE:
    default:
      return "NONE";
  }
}

struct SPbVirtualGoldMomentumPosition {
  bool isOpen;
  ENUM_PB_V501_POSITION_DIRECTION direction;
...
class CVirtualGoldMomentumPositionManager {
private:
  string m_symbol;
  double m_point;
  int m_digits;
  ...
  double m_dailyHoldingCostPerLot;
  ENUM_PB_V501_EXIT_MODE m_exitMode;

  SPbVirtualGoldMomentumPosition m_position;
  ...
  SPbV501PeriodStats m_yearStats[32];
  int m_yearStatsCount;
  SPbV501PeriodStats m_periodStats[3];
  ...
  void PrintPeriodStatsLine(
    ...
    const SPbV501PeriodStats &stats) const {
  ...
  bool CalculateVolumeByRisk(
    ...
    ENUM_ORDER_TYPE orderType =
      direction == PB_V501_POSITION_LONG
      ? ORDER_TYPE_BUY
      : ORDER_TYPE_SELL;
  ...
  bool ClosePosition(
    ...
    ENUM_ORDER_TYPE orderType =
      m_position.direction == PB_V501_POSITION_LONG
      ? ORDER_TYPE_BUY
      : ORDER_TYPE_SELL;
    ...
    double durationDays =
      (double)durationMinutes / 1440.0;

    double estimatedCosts =
      m_position.volumeLots *
      (
        m_commissionPerLotRoundTurn +
        m_dailyHoldingCostPerLot * durationDays
      );
    ...
    if (reason == PB_V501_EXIT_STOP_INITIAL)
      m_initialStopExitCount++;
    else if (reason == PB_V501_EXIT_MOMENTUM_REVERSAL)
      m_momentumExitCount++;
    else if (reason == PB_V501_EXIT_TEST_END)
      m_testEndExitCount++;
    ...
    PrintFormat(
      ...
      "Coûts=%.2f %s | Net=%.2f %s | %.2f R | "
      "Capital=%.2f %s | Durée=%d min",

      __FILE__,
      V501PositionDirectionToString(
        m_position.direction),

      V501ExitReasonToString(reason),
      m_digits,
      m_position.entryPrice,
      m_digits,
      exitPrice,
    ...
    m_position.direction = PB_V501_POSITION_FLAT;
    return true;
  }

public:
  CVirtualGoldMomentumPositionManager(void) {
    m_symbol = "";
    m_point = 0.0;
    m_digits = 0;
    m_currency = "";
    ...
    m_dailyHoldingCostPerLot = 0.0;
    m_exitMode = PB_V501_EXIT_MODE_MOMENTUM_REVERSAL;
  ...
  bool Initialize(
    ...
    m_dailyHoldingCostPerLot = dailyHoldingCostPerLot;
    m_exitMode = PB_V501_EXIT_MODE_MOMENTUM_REVERSAL;
  ...
  ENUM_PB_V501_POSITION_DIRECTION Direction(void) const {
  ...
  ENUM_PB_V501_EXIT_MODE ExitMode(void) const {
    return m_exitMode;
  }
  ...
  bool OpenPosition(
    ...
    if (!CalculateVolumeByRisk(
      ...
      PrintFormat(
        ...
        "SL=%.1f points",
        __FILE__,
        V501PositionDirectionToString(direction),
        m_virtualCapital,
        m_currency,
        targetRiskMoney,
        m_currency,
    ...
    if (direction == PB_V501_POSITION_LONG)
      m_longCount++;
    else
      m_shortCount++;
  ...
  bool ProcessTick(
    ...
    bool stopReached =
      ...
      ||
      (
        m_position.direction == PB_V501_POSITION_SHORT &&
        executablePrice >= m_position.stopLossPrice
      );
  ...
  bool ProcessMomentumExit(
    const ENUM_PB_V501_POSITION_DIRECTION momentumDirection,
    const MqlTick &tick) {

    if (!m_position.isOpen)
      return false;

    bool momentumReversed =
      momentumDirection != PB_V501_POSITION_FLAT &&
      momentumDirection != m_position.direction;
  ...
  void PrintAutomaticAnalysis(
    ...
    PrintFormat(
      ...
      "Résultat net=%.2f %s | Commission=%.2f %s/lot AR | "
      "Portage=%.2f %s/lot/jour",
      __FILE__,
      m_grossTotalMoney,
      m_currency,
  ...
  void PrintFinalSummary(void) const {
    ...
    PrintFormat(
      ...
      "Stop initial=%d | Inversion momentum=%d | Fin test=%d",
      __FILE__,
      m_openCount,
      m_closedCount,
      m_longCount,
    ...
    PrintFormat(
      ...
      V501ExitModeToString(m_exitMode),
      m_totalEstimatedCosts,
      m_currency);
  }
};
