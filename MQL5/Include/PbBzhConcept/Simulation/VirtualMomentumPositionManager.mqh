//+------------------------------------------------------------------+
//|             VirtualMomentumPositionManager.mqh                  |
//|  Stop ATR, inversion du momentum et simulation du capital       |
//+------------------------------------------------------------------+
#ifndef PB_BZH_VIRTUAL_MOMENTUM_POSITION_MANAGER_MQH
#define PB_BZH_VIRTUAL_MOMENTUM_POSITION_MANAGER_MQH

enum ENUM_PB_V5_POSITION_DIRECTION {
  PB_V5_POSITION_FLAT = 0,
  PB_V5_POSITION_LONG = 1,
  PB_V5_POSITION_SHORT = -1
};

enum ENUM_PB_V5_EXIT_MODE {
  PB_V5_EXIT_MODE_MOMENTUM_REVERSAL = 0
};

enum ENUM_PB_V5_EXIT_REASON {
  PB_V5_EXIT_NONE = 0,
  PB_V5_EXIT_STOP_INITIAL,
  PB_V5_EXIT_MOMENTUM_REVERSAL,
  PB_V5_EXIT_TEST_END
};

string V5PositionDirectionToString(
  const ENUM_PB_V5_POSITION_DIRECTION direction) {

  switch (direction) {
    case PB_V5_POSITION_LONG:
      return "LONG";

    case PB_V5_POSITION_SHORT:
      return "SHORT";

    case PB_V5_POSITION_FLAT:
    default:
      return "FLAT";
  }
}

string V5ExitModeToString(
  const ENUM_PB_V5_EXIT_MODE mode) {
  return "MOMENTUM_REVERSAL";
}

string V5ExitReasonToString(
  const ENUM_PB_V5_EXIT_REASON reason) {

  switch (reason) {
    case PB_V5_EXIT_STOP_INITIAL:
      return "STOP_INITIAL";

    case PB_V5_EXIT_MOMENTUM_REVERSAL:
      return "MOMENTUM_REVERSAL";

    case PB_V5_EXIT_TEST_END:
      return "TEST_END";

    case PB_V5_EXIT_NONE:
    default:
      return "NONE";
  }
}

struct SPbVirtualMomentumPosition {
  bool isOpen;
  ENUM_PB_V5_POSITION_DIRECTION direction;
...
class CVirtualMomentumPositionManager {
private:
  string m_symbol;
  double m_point;
  int m_digits;
  ...
  ENUM_PB_V5_EXIT_MODE m_exitMode;

  SPbVirtualMomentumPosition m_position;
  ...
  bool ClosePosition(
    ...
    ENUM_ORDER_TYPE orderType =
      m_position.direction == PB_V5_POSITION_LONG
      ? ORDER_TYPE_BUY
      : ORDER_TYPE_SELL;
    ...
    if (reason == PB_V5_EXIT_STOP_INITIAL)
      m_initialStopExitCount++;
    else if (reason == PB_V5_EXIT_MOMENTUM_REVERSAL)
      m_momentumExitCount++;
    else if (reason == PB_V5_EXIT_TEST_END)
      m_testEndExitCount++;
    ...
    PrintFormat(
      ...
      __FILE__,
      V5PositionDirectionToString(
        m_position.direction),

      V5ExitReasonToString(reason),
      m_digits,
      m_position.entryPrice,
      m_digits,
      exitPrice,
    ...
    m_position.direction = PB_V5_POSITION_FLAT;
    return true;
  }

public:
  CVirtualMomentumPositionManager(void) {
    m_symbol = "";
    m_point = 0.0;
    m_digits = 0;
    m_currency = "";
    ...
    m_exitMode = PB_V5_EXIT_MODE_MOMENTUM_REVERSAL;
  ...
  ENUM_PB_V5_POSITION_DIRECTION Direction(void) const {
  ...
  ENUM_PB_V5_EXIT_MODE ExitMode(void) const {
    return m_exitMode;
  }
  ...
  bool OpenPosition(
    ...
    if (m_position.isOpen ||
      (direction != PB_V5_POSITION_LONG &&
       direction != PB_V5_POSITION_SHORT)) {
  ...
  bool ProcessTick(
    ...
    bool stopReached =
      ...
      ||
      (
        m_position.direction == PB_V5_POSITION_SHORT &&
        executablePrice >= m_position.stopLossPrice
      );
  ...
  bool ProcessMomentumExit(
    const ENUM_PB_V5_POSITION_DIRECTION momentumDirection,
    const MqlTick &tick) {
    ...
    bool momentumReversed =
      momentumDirection != PB_V5_POSITION_FLAT &&
