//+------------------------------------------------------------------+
//|          VirtualGoldSwingRiskPositionManager.mqh                 |
//|  Momentum GOLD LONG, coûts et rapport de robustesse automatique |
//+------------------------------------------------------------------+
#ifndef PB_BZH_VIRTUAL_GOLD_SWING_RISK_POSITION_MANAGER_MQH
#define PB_BZH_VIRTUAL_GOLD_SWING_RISK_POSITION_MANAGER_MQH

enum ENUM_PB_V601_POSITION_DIRECTION {
  PB_V601_POSITION_FLAT = 0,
  PB_V601_POSITION_LONG = 1,
  PB_V601_POSITION_SHORT = -1
};

enum ENUM_PB_V601_EXIT_MODE {
  PB_V601_EXIT_MODE_SIGNAL_REVERSAL = 0
};

enum ENUM_PB_V601_EXIT_REASON {
  PB_V601_EXIT_NONE = 0,
  PB_V601_EXIT_STOP_INITIAL,
  PB_V601_EXIT_SIGNAL_REVERSAL,
  PB_V601_EXIT_TEST_END
};

string V601PositionDirectionToString(
  const ENUM_PB_V601_POSITION_DIRECTION direction) {

  switch (direction) {
    case PB_V601_POSITION_LONG:
      return "LONG";

    case PB_V601_POSITION_SHORT:
      return "SHORT";

    case PB_V601_POSITION_FLAT:
    default:
      return "FLAT";
  }
}

string V601ExitModeToString(
  const ENUM_PB_V601_EXIT_MODE mode) {
  return "SIGNAL_OR_TREND_REVERSAL";
}

string V601ExitReasonToString(
  const ENUM_PB_V601_EXIT_REASON reason) {

  switch (reason) {
    case PB_V601_EXIT_STOP_INITIAL:
      return "STOP_INITIAL";

    case PB_V601_EXIT_SIGNAL_REVERSAL:
      return "SIGNAL_OR_TREND_REVERSAL";

    case PB_V601_EXIT_TEST_END:
      return "TEST_END";

    case PB_V601_EXIT_NONE:
    default:
      return "NONE";
  }
}

string V601MetricTimeToString(
...
struct SPbVirtualGoldSwingRiskPosition {
  bool isOpen;
  ENUM_PB_V601_POSITION_DIRECTION direction;
...
class CVirtualGoldSwingRiskPositionManager {
private:
  string m_symbol;
  double m_point;
  int m_digits;
  ...
  ENUM_PB_V601_EXIT_MODE m_exitMode;

  SPbVirtualGoldSwingRiskPosition m_position;
  ...
  SPbV601PeriodStats m_yearStats[32];
  int m_yearStatsCount;
  SPbV601PeriodStats m_periodStats[3];
  ...
  bool CalculateOpenNetResult(
    ...
    ENUM_ORDER_TYPE orderType =
      m_position.direction == PB_V601_POSITION_LONG
      ? ORDER_TYPE_BUY
      : ORDER_TYPE_SELL;
  ...
  void UpdateEquityMetrics(
    ...
    if (drawdownPercent > m_maxEquityDrawdownPercent)
  ...
  void PrintPeriodStatsLine(
    ...
    const SPbV601PeriodStats &stats) const {
  ...
  bool CalculateVolumeByRisk(
    ...
    ENUM_ORDER_TYPE orderType =
      direction == PB_V601_POSITION_LONG
      ? ORDER_TYPE_BUY
      : ORDER_TYPE_SELL;
  ...
  bool ClosePosition(
    ...
    if (reason == PB_V601_EXIT_STOP_INITIAL)
      m_initialStopExitCount++;
    else if (reason == PB_V601_EXIT_SIGNAL_REVERSAL)
      m_signalExitCount++;
    else if (reason == PB_V601_EXIT_TEST_END)
      m_testEndExitCount++;
    ...
    PrintFormat(
      ...
      __FILE__,
      V601PositionDirectionToString(
        m_position.direction),

      V601ExitReasonToString(reason),
      m_digits,
      m_position.entryPrice,
      m_digits,
      exitPrice,
    ...
    m_position.direction = PB_V601_POSITION_FLAT;
    return true;
  }

public:
  CVirtualGoldSwingRiskPositionManager(void) {
    m_symbol = "";
    m_point = 0.0;
    m_digits = 0;
    m_currency = "";
    ...
    m_exitMode = PB_V601_EXIT_MODE_SIGNAL_REVERSAL;
  ...
  ENUM_PB_V601_POSITION_DIRECTION Direction(void) const {
  ...
  ENUM_PB_V601_EXIT_MODE ExitMode(void) const {
    return m_exitMode;
  }
  ...
  bool OpenPosition(
    ...
    if (!CalculateVolumeByRisk(
      ...
      PrintFormat(
        ...
        "SL=%.1f points",
        __FILE__,
        V601PositionDirectionToString(direction),
        m_virtualCapital,
        m_currency,
        targetRiskMoney,
        m_currency,
    ...
    if (direction == PB_V601_POSITION_LONG)
      m_longCount++;
    else
      m_shortCount++;
    ...
    UpdateOpenRiskMetricsAtPrice(
      direction == PB_V601_POSITION_LONG ? tick.bid : tick.ask,
      (datetime)tick.time);
  ...
  bool ProcessTick(
    ...
    bool stopReached =
      ...
      ||
      (
        m_position.direction == PB_V601_POSITION_SHORT &&
        executablePrice >= m_position.stopLossPrice
      );
  ...
  bool ProcessSignalExit(
    const ENUM_PB_V601_POSITION_DIRECTION signalDirection,
    const MqlTick &tick) {
    ...
    bool signalReversed =
      signalDirection != PB_V601_POSITION_FLAT &&
  ...
  void PrintAutomaticAnalysis(
    ...
    PrintFormat(
      ...
      "Pic équité=%.2f %s | Drawdown clôturé=%.2f %s (%.2f%%)",
      __FILE__,
    ...
    PrintFormat(
      ...
      m_currency,
      V601MetricTimeToString(m_worstTradeMaeEntryTime, TIME_DATE),
      V601MetricTimeToString(m_worstTradeMaeExitTime, TIME_DATE),
      m_bestTradeMfeMoney,
      m_currency,
      V601MetricTimeToString(m_bestTradeMfeEntryTime, TIME_DATE),
      V601MetricTimeToString(m_bestTradeMfeExitTime, TIME_DATE),
      averageMfeMoney,
      m_currency,
      m_maxGivebackMoney,
      m_currency,
      V601MetricTimeToString(m_maxGivebackEntryTime, TIME_DATE),
      V601MetricTimeToString(m_maxGivebackExitTime, TIME_DATE),
  ...
  void PrintFinalSummary(void) const {
    ...
    PrintFormat(
      ...
      V601ExitModeToString(m_exitMode),
      m_totalEstimatedCosts,
      m_currency);
  }
};
