//+------------------------------------------------------------------+
//|                VirtualTrendPositionManager.mqh                  |
//|  Stop ATR, Chandelier Exit et simulation du capital             |
//+------------------------------------------------------------------+
#ifndef PB_BZH_VIRTUAL_TREND_POSITION_MANAGER_MQH
#define PB_BZH_VIRTUAL_TREND_POSITION_MANAGER_MQH

enum ENUM_PB_V3_POSITION_DIRECTION {
  PB_V3_POSITION_FLAT = 0,
  PB_V3_POSITION_LONG = 1,
  PB_V3_POSITION_SHORT = -1
};

enum ENUM_PB_V3_EXIT_MODE {
  PB_V3_EXIT_CHANDELIER_ONLY = 0
};

enum ENUM_PB_V3_EXIT_REASON {
  PB_V3_EXIT_NONE = 0,
  PB_V3_EXIT_STOP_INITIAL,
  PB_V3_EXIT_CHANDELIER,
  PB_V3_EXIT_TEST_END
};

string V3PositionDirectionToString(
  const ENUM_PB_V3_POSITION_DIRECTION direction) {

  switch (direction) {
    case PB_V3_POSITION_LONG:
      return "LONG";

    case PB_V3_POSITION_SHORT:
      return "SHORT";

    case PB_V3_POSITION_FLAT:
    default:
      return "FLAT";
  }
}

string V3ExitModeToString(
  const ENUM_PB_V3_EXIT_MODE mode) {
  return "CHANDELIER_ONLY";
}

string V3ExitReasonToString(
  const ENUM_PB_V3_EXIT_REASON reason) {

  switch (reason) {
    case PB_V3_EXIT_STOP_INITIAL:
      return "STOP_INITIAL";

    case PB_V3_EXIT_CHANDELIER:
      return "CHANDELIER";

    case PB_V3_EXIT_TEST_END:
      return "TEST_END";

    case PB_V3_EXIT_NONE:
    default:
      return "NONE";
  }
}

struct SPbVirtualTrendPosition {
  bool isOpen;
...
class CVirtualTrendPositionManager {
private:
  string m_symbol;
  double m_point;
  int m_digits;
  ...
  ENUM_PB_V3_EXIT_MODE m_exitMode;
  double m_chandelierAtrMultiplier;

  SPbVirtualTrendPosition m_position;
  ...
  bool ClosePosition(
    ...
    ENUM_ORDER_TYPE orderType =
      m_position.direction == PB_V3_POSITION_LONG
      ? ORDER_TYPE_BUY
      : ORDER_TYPE_SELL;
    ...
    if (reason == PB_V3_EXIT_STOP_INITIAL)
      m_initialStopExitCount++;
    else if (reason == PB_V3_EXIT_CHANDELIER)
      m_chandelierExitCount++;
    else if (reason == PB_V3_EXIT_TEST_END)
      m_testEndExitCount++;
    ...
    PrintFormat(
      ...
      __FILE__,
      V3PositionDirectionToString(
        m_position.direction),

      V3ExitReasonToString(reason),
      m_digits,
      m_position.entryPrice,
      m_digits,
      exitPrice,
    ...
    m_position.direction = PB_V3_POSITION_FLAT;
    return true;
  }

public:
  CVirtualTrendPositionManager(void) {
    m_symbol = "";
    m_point = 0.0;
    m_digits = 0;
    m_currency = "";
    ...
    m_exitMode = PB_V3_EXIT_CHANDELIER_ONLY;
  ...
  ENUM_PB_V3_POSITION_DIRECTION Direction(void) const {
  ...
  ENUM_PB_V3_EXIT_MODE ExitMode(void) const {
    return m_exitMode;
  }
  ...
  bool OpenPosition(
    ...
    if (!CalculateVolumeByRisk(
      ...
      PrintFormat(
        ...
        "SL=%.1f points",
        __FILE__,
        V3PositionDirectionToString(direction),
        m_virtualCapital,
        m_currency,
        targetRiskMoney,
        m_currency,
    ...
    if (direction == PB_V3_POSITION_LONG)
      m_longCount++;
    else
      m_shortCount++;
  ...
  bool ProcessTick(
    ...
    if (m_position.direction == PB_V3_POSITION_LONG) {
    ...
    if (m_position.currentAtrPrice > 0.0) {
      ...
      bool improvesStop =
        ...
        ||
        (
          m_position.direction == PB_V3_POSITION_SHORT &&
          candidateStop < m_position.stopLossPrice
        );
    ...
    ENUM_PB_V3_EXIT_REASON exitReason =
      m_position.chandelierActivated
      ? PB_V3_EXIT_CHANDELIER
      : PB_V3_EXIT_STOP_INITIAL;
