//+------------------------------------------------------------------+
//|          VirtualGoldSwingBreakoutReentryPositionManager.mqh      |
//|  Momentum GOLD LONG, coûts et rapport de robustesse automatique |
//+------------------------------------------------------------------+
#ifndef PB_BZH_VIRTUAL_GOLD_SWING_BREAKOUT_REENTRY_POSITION_MANAGER_MQH
#define PB_BZH_VIRTUAL_GOLD_SWING_BREAKOUT_REENTRY_POSITION_MANAGER_MQH

enum ENUM_PB_V603_POSITION_DIRECTION {
  PB_V603_POSITION_FLAT = 0,
  PB_V603_POSITION_LONG = 1,
  PB_V603_POSITION_SHORT = -1
};

enum ENUM_PB_V603_EXIT_MODE {
  PB_V603_EXIT_MODE_SIGNAL_REVERSAL = 0
};

enum ENUM_PB_V603_EXIT_REASON {
  PB_V603_EXIT_NONE = 0,
  PB_V603_EXIT_STOP_INITIAL,
  PB_V603_EXIT_PROFIT_PROTECTION,
  PB_V603_EXIT_SIGNAL_REVERSAL,
  PB_V603_EXIT_TEST_END
};

string V603PositionDirectionToString(
  const ENUM_PB_V603_POSITION_DIRECTION direction) {

  switch (direction) {
    case PB_V603_POSITION_LONG:
      return "LONG";

    case PB_V603_POSITION_SHORT:
      return "SHORT";

    case PB_V603_POSITION_FLAT:
      default:
      return "FLAT";
  }
}

string V603ExitModeToString(
  const ENUM_PB_V603_EXIT_MODE mode) {
  return "SIGNAL_OR_TREND_REVERSAL";
}

string V603ExitReasonToString(
  const ENUM_PB_V603_EXIT_REASON reason) {

  switch (reason) {
    case PB_V603_EXIT_STOP_INITIAL:
      return "STOP_INITIAL";

    case PB_V603_EXIT_PROFIT_PROTECTION:
      return "PROFIT_PROTECTION";

    case PB_V603_EXIT_SIGNAL_REVERSAL:
      return "SIGNAL_OR_TREND_REVERSAL";

    case PB_V603_EXIT_TEST_END:
      return "TEST_END";

    case PB_V603_EXIT_NONE:
      default:
      return "NONE";
  }
}

string V603MetricTimeToString(
...
struct SPbVirtualGoldSwingReentryPosition {
  bool isOpen;
  ENUM_PB_V603_POSITION_DIRECTION direction;
...
class CVirtualGoldSwingReentryPositionManager {
  private:
    string m_symbol;
  double m_point;
  int m_digits;
  ...
  ENUM_PB_V603_EXIT_MODE m_exitMode;
  ENUM_PB_V603_EXIT_REASON m_lastExitReason;
  double m_lastProfitProtectionPeakPrice;

  SPbVirtualGoldSwingReentryPosition m_position;
  ...
  SPbV603PeriodStats m_yearStats[32];
  int m_yearStatsCount;
  SPbV603PeriodStats m_periodStats[3];
  ...
  bool CalculateOpenNetResult(
    ...
    ENUM_ORDER_TYPE orderType =
      m_position.direction == PB_V603_POSITION_LONG
      ? ORDER_TYPE_BUY
      : ORDER_TYPE_SELL;
  ...
  void UpdateProfitProtectionAtPrice(
    ...
    bool improvesStop =
      ...
      ||
      (
      m_position.direction == PB_V603_POSITION_SHORT &&
      candidateStop < m_position.stopLossPrice
    );
  ...
  void PrintPeriodStatsLine(
    ...
    const SPbV603PeriodStats &stats) const {
  ...
  bool ClosePosition(
    ...
    if (reason == PB_V603_EXIT_STOP_INITIAL)
    m_initialStopExitCount++;
    else if (reason == PB_V603_EXIT_PROFIT_PROTECTION)
    m_profitProtectionExitCount++;
    else if (reason == PB_V603_EXIT_SIGNAL_REVERSAL)
    m_signalExitCount++;
    else if (reason == PB_V603_EXIT_TEST_END)
    m_testEndExitCount++;
    ...
    PrintFormat(
      ...
      __FILE__,
      V603PositionDirectionToString(
        m_position.direction),

      V603ExitReasonToString(reason),
      m_digits,
      m_position.entryPrice,
      m_digits,
      exitPrice,
    ...
    if (reason == PB_V603_EXIT_PROFIT_PROTECTION) {
      m_lastProfitProtectionPeakPrice =
        m_position.highestExecutablePrice;
    }

    ZeroMemory(m_position);
    m_position.direction = PB_V603_POSITION_FLAT;
    return true;
  }

  public:
    CVirtualGoldSwingReentryPositionManager(void) {
    m_symbol = "";
    m_point = 0.0;
    m_digits = 0;
    m_currency = "";
    ...
    m_currentAtrPrice = 0.0;
    m_exitMode = PB_V603_EXIT_MODE_SIGNAL_REVERSAL;
    m_lastExitReason = PB_V603_EXIT_NONE;
    m_lastProfitProtectionPeakPrice = 0.0;
  ...
  ENUM_PB_V603_POSITION_DIRECTION Direction(void) const {
  ...
  ENUM_PB_V603_EXIT_MODE ExitMode(void) const {
    return m_exitMode;
  }
  ...
  ENUM_PB_V603_EXIT_REASON LastExitReason(void) const {
    return m_lastExitReason;
  }

  double LastProfitProtectionPeakPrice(void) const {
    return m_lastProfitProtectionPeakPrice;
  }
  ...
  bool OpenPosition(
    ...
    m_position.highestExecutablePrice =
      direction == PB_V603_POSITION_LONG ? tick.bid : tick.ask;
  ...
  bool ProcessSignalExit(
    const ENUM_PB_V603_POSITION_DIRECTION signalDirection,
    const MqlTick &tick) {
    ...
    bool signalReversed =
      signalDirection != PB_V603_POSITION_FLAT &&
  ...
  void PrintAutomaticAnalysis(
    ...
    PrintFormat(
      ...
      m_currency,
      V603MetricTimeToString(m_worstTradeMaeEntryTime, TIME_DATE),
      V603MetricTimeToString(m_worstTradeMaeExitTime, TIME_DATE),
      m_bestTradeMfeMoney,
      m_currency,
      V603MetricTimeToString(m_bestTradeMfeEntryTime, TIME_DATE),
      V603MetricTimeToString(m_bestTradeMfeExitTime, TIME_DATE),
      averageMfeMoney,
      m_currency,
      m_maxGivebackMoney,
      m_currency,
      V603MetricTimeToString(m_maxGivebackEntryTime, TIME_DATE),
      V603MetricTimeToString(m_maxGivebackExitTime, TIME_DATE),
  ...
  void PrintFinalSummary(void) const {
    ...
    PrintFormat(
      ...
      V603ExitModeToString(m_exitMode),
