//+------------------------------------------------------------------+
//|                                              PbBzhConceptEA.mq5  |
//|              Version 1.08 - Observateur avec signaux graphiques   |
//+------------------------------------------------------------------+
...
input ENUM_PB_VIRTUAL_VOLUME_MODE InpVirtualVolumeModeV108= PB_VIRTUAL_VOLUME_RISK_PERCENT;
...
int OnInit(void)
  {
   ...
   if(!g_virtualPositions.Init(
     ...
     g_logger.Info(
      "Initialisation de la version 1.04 "
      "en mode SIMULATION VIRTUELLE.");
   ...
   g_logger.Warning(
      ...
      g_timeframe=(InpSignalTimeframe==PERIOD_CURRENT)
                  ? (ENUM_TIMEFRAMES)_Period
                  : InpSignalTimeframe;

   if(!g_barClock.Init(_Symbol,g_timeframe))
     {
      g_logger.Error("Initialisation de l'horloge de bougies impossible.");
      return INIT_FAILED;
     }

   if(!g_signal.Init(_Symbol,
                     g_timeframe,
                     InpMaPeriod,
                     InpMaShift,
                     InpMaMethod,
                     InpMaAppliedPrice))
     {
      g_logger.Error("Initialisation du signal de moyenne mobile impossible.");
      return INIT_FAILED;
     }

   if(!g_renderer.Init(ChartID(),
                       InpExpertName,
                       _Symbol,
                       g_timeframe,
                       InpShowSignalArrows,
                       ...
                       InpSellArrowColor))
     {
      g_logger.Error("Initialisation de l'affichage graphique impossible.");
      return INIT_FAILED;
     }

   g_logger.Info(StringFormat("Prêt : symbole=%s, période=%s, MA=%d, flèches=%s.",
                              _Symbol,
...
void OnDeinit(const int reason)
  {
   ...
   g_logger.Info(StringFormat("Résumé positions virtuelles : %s",g_virtualPositions.BuildSummary()));
   g_signal.Deinit();
